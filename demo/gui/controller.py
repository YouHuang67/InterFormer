...
from . import predictor, clicker
from .utils import draw_with_blend_and_clicks


class InteractiveController:
    def __init__(self, net, device, update_image_callback, predictor_params=None, prob_thresh=0.5):
        # model and interactive record
        self.net = net
        self.device = device
        self.predictor = None
    ...
    def add_click(self, x, y, is_positive):
        ...
        self.predictor.update_ref_label_by_new_click(click)
        pred = self.predictor.get_prediction(len(self.clicker), prev_mask=self._init_mask)
    ...
    def reset_predictor(self, predictor_params=None):
        ...
        if self.predictor is None:
            self.predictor = predictor.Predictor(model=self.net, device=self.device, predictor_params=self.predictor_params)
        else:
            self.predictor.partially_reset_predictor(self.predictor_params)
        if self.image is not None and self.feature_changed:
